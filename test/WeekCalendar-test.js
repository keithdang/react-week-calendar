...
describe("WeekCalendar", () => {
  ...
  it("render", () => {
    ...
    expect(wrapper.find(".weekCalendar__scaleHeader").text()).equal(
      props.scaleHeaderTitle
    );
    expect(wrapper.find(ScaleColumn)).to.have.length(1);
    expect(wrapper.find(ScaleColumn).prop("scaleIntervals")).to.have.length(
      (24 * 60) / props.scaleUnit
    );
  ...
  it("calculate column dimension", () => {
    ...
    expect(wrapper.state("columnDimensions")[0]).to.deep.equal({
      width: 100,
      left: 0
    });
    expect(
      wrapper.state("columnDimensions")[props.numberOfDays - 1]
    ).to.deep.equal({ width: 100, left: (props.numberOfDays - 1) * 100 });
  });
  ...
  it("recalculate intervals when receive new props", () => {
    ...
    expect(wrapper.state("scaleIntervals")).to.deep.equal(
      CalendarUtils.getIntervalsByDuration(
        newProps.scaleUnit,
        moment({ h: 0, m: 0 }),
        moment({ h: 23, m: 59 })
      )
  ...
  it("handle scroll", () => {
    ...
    expect(wrapper.find(".weekCalendar__header").prop("style")).to.deep.equal({
      left: -200
    });
    expect(
      wrapper.find(".weekCalendar__scaleColumn").prop("style")
    ).to.deep.equal({ top: -100 });
  });
  ...
  it("update state on mouse enter", () => {
    ...
    wrapper.setState({
      columnDimensions,
      startSelectionPosition: startPosition,
      mousePosition: startPosition
    });
    wrapper
      .find(CalendarBody)
      .props()
      .onCellMouseEnter(1, 1);
    expect(wrapper.state("mousePosition")).to.deep.equal({ x: 1, y: 1 });
    expect(wrapper.state("startSelectionPosition")).to.deep.equal(
      startPosition
    );
  });
  ...
  it("mouseup - right click", () => {
    ...
    const wrapper = mount(
      <WeekCalendar {...props} onIntervalSelect={onIntervalSelect} />
    );
